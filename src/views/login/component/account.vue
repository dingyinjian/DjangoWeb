<template>
  <el-form ref="loginFormRef" size="large" :model="state.loginForm" :rules="state.rules" class="login-content-form">
    ...
    <el-form-item class="login-animation-three">
      <el-row :gutter="15">
        <el-col :span="16">
          <el-input type="text" maxlength="6" :placeholder="$t('message.account.accountPlaceholder3')"
            v-model="state.loginForm.captcha" clearable autocomplete="off">
            <template #prefix>
              <el-icon class="el-input__icon">
                <elementPosition />
              </el-icon>
...
import { captcha, signIn } from "@/api/login";
...
const getCaptcha = async () => {
  ...
  state.captchaImage = res.data.image_base;
  state.loginForm.captchaKey = res.data.key;
};
...
const onSignIn = async () => {
  ...
  console.log(userInfo.role.role_key === 'platformUser')
  if (userInfo.role.role_key === 'platformUser') {
    router.replace('/plateForm')
  } else {
...
const signInSuccess = () => {
  ...
  if (route.query?.redirect&& route.query?.redirect !== "/plateForm") {
    router.push({
